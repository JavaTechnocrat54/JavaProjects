...
public class CustomerChangePassword extends HttpServlet {
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ResultSet resultset = statement.executeQuery("select upass from tblcustomer where pass='" + cpassword+ "' and name='" + session.getAttribute("uname") + "'");
			if (resultset.next()) {
				pass = resultset.getString(1);
			}
			if (password.equals(confpass)) {
				if (pass.equals(cpassword)) {
					int i = statement.executeUpdate("update tblcustomer set pass='" + password + "' where name='"+ session.getAttribute("uname") + "' ");
