...
public class EmployLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			String vercode = request.getParameter("varcode");
			System.out.println(email+pass+ vercode);
			...
			if (captchaDB.equals(vercode)) {
				ResultSet resultset = st.executeQuery("select * from tblemployee where email='" + email + "' AND upass='" + pass + "'");
				if (resultset.next()) {
