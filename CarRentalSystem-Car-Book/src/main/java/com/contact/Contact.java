...
public class Contact extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			int help=DatabaseConnection.insertUpdateFromSqlQuery("insert into tblcontact(contact_id,name,email,mobile,subject,message)values('"+id+"','"+fullname+"','"+email+"','"+mobile+"','"+subject+"','"+message+"')");
			if(help>0) {
