...
public class CustomerEditProfile extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			int editProfile=DatabaseConnection.insertUpdateFromSqlQuery("update tblcustomer set cname='"+cname+"',mobile='"+mobile+"',gender='"+gender+"',dob='"+dob+"',address='"+address+"' where  id='"+customerId+"'");
			if(editProfile>0) {
				String message="Profile updated successfully.";
				session.setAttribute("profile-update", message);
				response.sendRedirect("customerViewProfile.jsp");
			}
		}catch(Exception e) {
			e.printStackTrace();
		}
